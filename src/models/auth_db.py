...
Multi-tenant with JWT + refresh tokens
"""
...
class AuthDatabase:
    ...
    def create_user(self, email: str, password: str, tenant_id: int = None) -> User:
        ...
        self.conn.commit()
        return User(
            id=cursor.lastrowid, email=email,
            password_hash=password_hash, salt=salt, tenant_id=tenant_id
        )
    ...
    def generate_refresh_token(self, user: User) -> str:
        payload = {
            ...
            'exp': int(time.time()) + JWT_REFRESH_EXPIRY
        }
        return jwt.encode(payload, JWT_SECRET, algorithm='HS256')
